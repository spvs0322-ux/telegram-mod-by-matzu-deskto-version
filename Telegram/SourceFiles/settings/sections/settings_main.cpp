...
void BuildMatzuProSection(SectionBuilder &builder) {
	const auto controller = builder.controller();
	const auto showOther = builder.showOther();
	...
	builder.addButton({
		.id = u"main/matzu_pro"_q,
		.title = rpl::single(QString("PRO Mode by Matzu")),
		.icon = { .icon = &st::menuIconManage, .newBadge = true },
		.label = Matzu::Config().proModeValue() | rpl::map([](bool on) {
			return on ? QString("ON") : QString("OFF");
		}),
		.onClick = [=] {
			showOther(MatzuProId());
		},
		.keywords = { u"matzu"_q, u"pro"_q, u"ghost"_q, u"privacy"_q, u"download"_q, u"viewonce"_q },
	});
